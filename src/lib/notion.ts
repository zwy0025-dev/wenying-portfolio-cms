...
// 数据库 ID（去掉连字符的格式）
const DB_IDS = {
  workExperience: process.env.NOTION_WORK_DB_ID?.replace(/-/g, '') || '',
  projectExperience: process.env.NOTION_PROJECT_DB_ID?.replace(/-/g, '') || '',
  aiLab: process.env.NOTION_AI_LAB_DB_ID?.replace(/-/g, '') || '',
  coreSkills: process.env.NOTION_SKILLS_DB_ID?.replace(/-/g, '') || '',
};
...
export async function getWorkExperiences(): Promise<WorkExperience[]> {
  if (!DB_IDS.workExperience) {
    console.log('Work DB ID not set');
    return [];
  }
...
export async function getProjectExperiences(): Promise<ProjectExperience[]> {
  if (!DB_IDS.projectExperience) {
    console.log('Project DB ID not set');
    return [];
  }
...
export async function getAILab(): Promise<AILab[]> {
  if (!DB_IDS.aiLab) {
    console.log('AI Lab DB ID not set');
    return [];
  }
...
export async function getCoreSkills(): Promise<CoreSkill[]> {
  if (!DB_IDS.coreSkills) {
    console.log('Skills DB ID not set');
    return [];
  }
